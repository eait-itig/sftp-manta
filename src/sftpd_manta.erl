...
-include_lib("ssh/src/ssh_connect.hrl"). %% For ?DEFAULT_PACKET_SIZE and ?DEFAULT_WINDOW_SIZE
...
init(Options) ->
    ...
    Vsn = proplists:get_value(sftpd_vsn, Options, 3),
...
handle_msg({ssh_channel_up, ChannelId,  ConnectionManager}, 
       #state{xf = Xf,
         options = Options,
         file_handler = FileMod,
         file_state = FS1} = State0) ->
    [{user, User}] = ssh_connection_handler:connection_info(ConnectionManager,
        [user]),
    FS2 = case erlang:function_exported(FileMod, login, 2) of
        true -> FileMod:login(User, FS1);
        _ -> FS1
    end,
    {{ok, NewCwd}, FS3} = FileMod:get_cwd(FS2),
    State = State0#state{cwd = NewCwd, file_state = FS3},
...
nattr_to_longname(NameUC, Attr) ->
    #ssh_xfer_attr{type = T, size = S, owner = O, group = G,
        permissions = Perms, mtime = MTime} = Attr,
    Type = case T of
        directory -> "d";
        symlink -> "l";
        _ -> "-"
    end,
    PermString = lists:foldl(fun ({BitN, Char}, Acc) ->
        case (Perms band (1 bsl BitN)) of
            0 -> [$- | Acc];
            _ -> [Char | Acc]
        end
    ...
    [Type, PermString, "   1  ", integer_to_binary(O), "     ",
        integer_to_binary(G), "     ", SBinPad, SBin, "  ",
        MTimeString, " ", NameUC].
...
encode_name(Vsn, {NameUC,Attr}, Len) when Vsn =< 3 ->
    ...
    NLen = length(Name),
    LongName = iolist_to_binary(nattr_to_longname(NameUC, Attr)),
    LNLen = byte_size(LongName),
    EncAttr = ssh_xfer:encode_ATTR(Vsn, Attr),
    ALen = size(EncAttr),
...
read_dir(State0 = #state{file_handler = FileMod, max_files = MaxLength, file_state = FS0},
    ...
    length(Files) > MaxLength ->
        ...
        xf_send_names(XF, ReqId, NamesAndAttrs),
...
stat(ReqId, RelPath, State0=#state{file_handler=FileMod, 
    ...
    case Res of
    {ok, FileInfo} ->
        Attr = ssh_sftp:info_to_attr(FileInfo),
        ssh_xfer:xf_send_attr(XF, ReqId, Attr),
        State1;
    {error, E} ->
